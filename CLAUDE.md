...
The frontend runs on port 3000, the backend on port 8000. In production both deploy to Vercel (frontend) and TBD (backend).
...
- Station types: R (river), L (lake/reservoir), M (meteorological)
- Readings refresh on demand via the frontend or admin endpoints
